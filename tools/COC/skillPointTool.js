...
const allocateSkillPoint = async ({ gameId, occupationSkills, totalPoint }) => {
  try {
    ...
    for (const [key, skillData] of Object.entries(occupationSkills)) {
      ...
      items[key] = {
        // 使用英文 key 作為 items 物件的 key，方便後續操作
    ...
    return {
      toolResult: {
        result: "success",
        message: `Please tell player to kick the infor icon on the right top.\nThe result you get it later which is the final point of those skills.`,
      },
    ...
    io.to(gameId).emit("system:error", {
      functionName: "allocateSkillPoint",
      error: e.message,
    });
    return {
      toolResult: {
  ...
  parameters: {
    ...
    properties: {
      occupationSkills: {
        ...
        additionalProperties: {
          ...
          properties: {
            displayLabel: {
              type: Type.STRING,
              description:
                "技能的顯示名稱。此字串必須與玩家當前的對話語言完全一致。例如，如果玩家使用英文，這裡就必須是英文。",
            },
            baseValue: {
              type: Type.NUMBER,
              description: "技能的基礎值。",
        ...
        description:
          '一個包含所有職業技能的物件。物件的鍵(key)是技能的英文程式碼，值(value)是一個包含 displayLabel 和 baseValue 的物件，最少6個，最多10個。範例 (當玩家說英文時): {"ArtCraft_Acting": {"displayLabel": "Art/Craft (Acting)", "baseValue": 5}, "Charm": {"displayLabel": "Charm", "baseValue": 15}}',
      },
      totalPoint: {
        type: Type.NUMBER,
